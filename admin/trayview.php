...
<table class="table table-bordered" id="resultTable" data-responsive="table" style="text-align: left;">
	<thead>
		<tr style="background: #0AFF00; text-align:center;">
			...
			<th width="20%"> Raw Cost </th>
			<th width="20%"> R.R.P </th>
			<th width="20%"> Profit </th>
			<th width="20%"> R.O.I </th>
	...
	<tbody>
			...
			?>
			<div class="profilePicture"><img src="images/<?php echo $row['ptImage']; ?>" width="250px" height="250px" alt="<?php echo $productName; ?>"</div>
	...
	<tbody>
		<?php
				...
				for( $i = 0; $items = $itemQ->fetch(); $i++ ){
					...
					$ptNameQ = $conn->prepare("SELECT ptName FROM producttype WHERE `ptid` = '$ptid'");
					$ptNameQ->execute();
					$varietyName = $ptNameQ->fetch();
					
					$uomQ = $conn->prepare("SELECT uomLong FROM uom WHERE `uom_id` = '$uom_id'");
					$uomQ->execute();
					$uom = $uomQ->fetch();
					
					?>
					<td><?php echo $varietyName['ptName']; ?></td>
					<td><?php echo $quantity; ?></td>
					<td><?php echo $uom['uomLong']; ?></td>	
					<td><?php echo ''; ?></td>				
					<td><?php echo $notes; ?></td>		
					<td>
						<a title="Click To Edit Item" rel="facebox" href="editItem.php?id=<?php echo $tray_id; ?>&ptid=<?php echo $ptid; ?>">
							<button class="btn btn-warning btn-mini">
								<i class="icon-edit"></i> Edit  
							</button>
						</a>
