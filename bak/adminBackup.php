...
<?php
		  ...
		  if($categoryR !== false) {
			foreach( $categoryR as $category){
				$cid = $category['cid'];
				echo '<button class="accordion" style="background-color: lightgreen; font-style:bold; font-size: 26px">' . $category['categoryName'];
				echo '&nbsp<span style="background-color : #90ef50; font-size: 26px;">Average Margin at ' . $category['categoryMargin'] . '%</span></button>';
				...
				if($productR !== false) {
					foreach($productR as $product) {
						$pid = $product['pid'];
						echo '<button class="accordion" style="background-color: #FCD299;">' . $product['productName'];
						echo '&nbsp<span style="background-color : #90ef50; font-size: 26px;">Auto Margin at ' . $product['productMargin'] . '%</span></button>';
						echo '<div class="panel">';
						
						$productTypeQ = "SELECT * FROM `producttype` WHERE `pid` = '$pid' && `status` = '1' && `archive` = '0'";
						...
						if($productTypeR !== false) {
							...
							foreach($productTypeR as $productType) {
								...
								echo '<div class="rrp" style="text-align: center;">$' . $productType['ptPrice'] . '</div>';
								
								echo '<div class="productAmount" style="text-align: center;"><input type="number" size="5" step="1" min="0" value="" name="' . $ptid . '" id="' . $ptid .'" onClick="update( id, value )"></div>';
