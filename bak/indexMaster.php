...
<body>
  ...
  <div id="content">
    <span class="slide">
	  ...
	  <button class="accordion" style="background-color: #FCD299;"><i class="fas fa-shopping-cart"></i></button>
	  <div class="panel">
		  <p>
			<div id="parent">
    		  <div id="child-left">
...
<?php
		  ...
		  if($categoryR !== false) {
			foreach( $categoryR as $category){
				$cid = $category['cid'];
				echo '<button class="accordion" style="background-color: lightgreen; font-style:bold; font-size: 26px">' . $category['categoryName'] . '</button>';
				...
				if($productR !== false) {
					foreach($productR as $product) {
						$pid = $product['pid'];
						echo '<button class="accordion" style="background-color: #FCD299;;">' . $product['productName'] . '</button>';
						echo '<div class="panel">';
