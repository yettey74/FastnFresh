...
if ( isset( $_GET[ 'cid' ] ) ){
	...
	$categoryQ = "SELECT * FROM `category` WHERE `cid` = '$cid'";
	$categoryR = $conn->query( $categoryQ );
	
	if ( $categoryR != FALSE ){
	  $category = $categoryR->fetch();
...
<?php	
	...
	echo'<tr><td><input type="submit" value="Submit" id="product-submit" name="product-submit"><input type="button" value="No, go back!" onclick="history.go(-1)"></td></tr>';
	echo '</table>';				
	echo '</form>';	
?>
